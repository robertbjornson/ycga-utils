...
'''
This is for the HS4000, which takes a totally different samplesheet, akin to the one produced by the IEM.  It should look like:
        ...
        if not m:
            ...
            if x2:
                if x1 not in index2barcode or x2 not in index2barcode:
                    ...
                    bcx, bcseq = '%03d_%03d'%(int(x1), int(x2)), index2barcode[x1] + "," + rc.rc(index2barcode[x2])
            else:
